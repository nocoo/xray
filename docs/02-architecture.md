...
Channel keys extend the existing hashed-token store with a required channel binding and the `articles:write` scope. They cannot read the watchlist graph or submit watchlist items. Browser-only channel routes manage names, keys, tags, and reports. Articles live in their own D1 table, ordered by report date rather than ingestion time. Producer retries use `(channel_id, external_id)` deduplication and cannot overwrite content. Browser owners may edit content while preserving the external ID and source, or delete a report; deletion releases its external ID for a future submission. Channel and key tags reuse the tenant tag catalog with atomically replaced join-table associations. The UI renders stored Markdown and loads HTTPS image links directly; no image storage or server-side image fetching is involved. The full DTO and route matrix is in [Channels](11-channels.md).
...
| Worker local (Mock) | 37007 behind Caddy → public 7007 | `.wrangler/state-mock` |
