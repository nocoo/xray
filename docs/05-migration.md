...
4. Add **staging hostnames** to Worker allowlist: `xray-staging.hexly.ai` (Access), `xray-ingest-staging.worker.hexly.ai` (bypass) — for smoke only.  
...
13. **Ingest DNS** `xray-ingest.worker.hexly.ai` → smoke push.  
