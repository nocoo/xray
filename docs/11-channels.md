...
| GET | `/api/channels/:id/articles` | Query `date_from=YYYY-MM-DD`, `date_to=YYYY-MM-DD`, `q`, `tag_ids=<comma-separated ids>`, `before=<article id>`, `limit` (default 30, max 100); `ArticlePage` |
...
Article summaries and details expose `tags: Tag[]`: a live union of channel tags and source-token tags, deduplicated by ID and sorted by name, then ID. Revocation preserves the token association; renaming/deleting tags updates existing articles. Tags appear only beneath the open article title, with no duplicates in the page toolbar or article list. No additional migration or stored snapshot is needed.
...
Routes: `/channels/:channelId`, `/channels/:channelId/articles/:articleId`. Entering a channel selects its first report after the matching list loads; explicit article links stay selected. The list and document scroll independently. On narrow screens, show one pane at a time. URL navigation preserves the channel, combined filters and selection; session-local reading positions restore on back/forward and reload. New requests cannot overwrite a later navigation. Loading or errors must not erase an already open report.

The list toolbar combines keyword search with a Basalt filter Popover for inclusive date ranges and up to 20 tags. Filters are applied server-side before cursor pagination: dates, keyword and tags combine with AND, while selected tags match ANY tag in the live channel/source union. Keywords are literal title/summary/Markdown substrings, trimmed and limited to 200 characters; SQLite ASCII case folding applies, and non-ASCII text matches literally. Date endpoints may be open; reversed or invalid ranges fail validation. Tag IDs are deduplicated positive safe integers. Changing filters selects the first matching report; on mobile the list and filter focus remain visible. Editing a report out of the active results selects the first remaining match. Reading positions and bounded page restoration are scoped to the complete filter query. Popovers scroll within the available viewport height.
...
Typography follows Kami and GeekHub: Chinese `TsangerJinKai02`, English Charter, 18px body, 1.65 line height, maximum prose width 1020px, or full available width. Interface text stays sans serif; code stays monospace. Reuse GeekHub's local WOFF2 subsets and preserve its font notice; these font assets are not MIT licensed. Provide serif/sans, size and width preferences. PageHeader places outlined Lucide controls for font, size, width and channel settings in one row at the top right; date controls live in the list filter Popover, with no floating reader toolbar. Keep the title and byline compact without reserved toolbar clearance. PageHeader supplies the page title and subtitle. Channel management and settings use the brightest Basalt surface for statistics and restrained Lucide accents for titles, metrics and sections.

Below 768px, the channel heading and description each occupy one truncated line, followed by a compact action row. Hide the decorative channel icon and desktop font-family/full-width controls; retain report return, edit/delete, font size, related links and channel settings. Long channel metadata must not consume the document viewport. Browser back/forward reopens the routed article even after returning to the report list; filter navigation still keeps the mobile list visible. The article title wraps in the independently scrolling document.
...
Related links open by default beside the document in both standard and full-width reading. Above 920px of content width, both side panels share a responsive 240–345.6px token-based width, keeping the central document wider, with matching rounded borders, aligned headings and subdued Basalt count badges. The three cards share one top edge with a 12px gap, independently scrolling content, quieter side surfaces and the brightest central document. At 920px or below, the report list and reading view alternate; links stay beside the document until content width reaches 560px, where they follow the article in its natural scrolling flow. Links never use an overlay or drawer.

The panel close button returns focus to the header link icon, which reopens it and includes the source count in its tooltip. Closing releases its width on desktop or its height below the article; transitions respect reduced-motion preferences. Hidden links remain inert, retaining loaded previews for reopening. The manual choice survives article navigation within the reader; reloading starts expanded. Reader shortcuts do not intercept interactions inside the related-links panel.

Empty lists and documents use centered Basalt empty states with Lucide icons. Copy distinguishes a channel awaiting its first report from filters with no matches; failed requests offer retry actions. Unavailable link-preview metadata preserves the extracted source label and URL rather than hiding the link.
...
### Local reading, tag catalog and Mock expansion — 2026-09-22

The Tags catalog now has a compact six-color filter with actual counts, name search/sorting, aligned bright tiles, truncated long names and standard Basalt creation/rename/deletion dialogs. Related-link previews are integrated with standard/full-width reading, inline mobile reading and keyboard focus. Header mode labels are **MOCK** / **PROD**.

The idempotent local seed contains 14 channels, 45 tags, 140 reports and 50 named fixture tokens (33 active and 17 revoked). Each channel has at least eight reports; R&D has 33 for pagination. There are 134 distinct Markdown bodies, covering multilingual and long labels, source/channel tag overlap, no tags, multiple source states, long/short text, tables, tasks, code, external images, reference links and missing previews. Fixture tokens have deliberately unusable hashes. Stable IDs and ownership checks preserve edited/user-created resources. The original empty-channel placeholder is renamed only when its original name and description both match. Compact SQL tuples keep the seed below the local D1 SQL size limit without shortening report content.

The same catalog now includes 13 watchlists with 108 members and 236 timeline items, plus 13 groups with 106 members and 59 ingest logs. Every watchlist contains at least eight members and 16 items; every group contains at least eight members. Fictional X and custom sources cover untagged and multi-tag members, replies, quotes, reposts, photos, video/GIF, long text, external source URLs and all four AI states. Shared handles exercise group-copy deduplication; ingest totals include accepted, deduplicated and rejected samples. New watchlists disable automatic translation and require no live credentials. All 208 added canonical payloads pass strict parsing; repeat imports preserve user edits and preferences. Native local D1 import succeeded, and the Caddy API confirmed these counts while the dev server remained running.

Validation: 809 unit tests passed (170 shared, 461 Worker, 178 UI), including native workerd HTML parsing and the bounded SQL fixture. Strict types, lint and production build passed; the existing Vite bundle-size warning remains. Coverage retains its original scope and all four 95% floors: shared S98.31/B96.19/F100/L99.20, Worker S98.61/B95.92/F97.19/L99.70, UI S99.50/B97.20/F99.61/L99.88. Isolated L2 passed 33 real HTTP tests with 60/60 routes; isolated L3 passed all 20 browser journeys with no retries. L2 ran from a temporary source snapshot so its shared-path harness could not interrupt the daily dev server. All test stores and task servers were removed after validation. Live read-only preview requests through Caddy retrieved Kami and Cloudflare Open Graph titles, descriptions and external images. Local D1 import and repeated import both passed; Caddy verified 14 channels, 45 tags and 140 reports. The dev server remains online at `https://xray.dev.hexly.ai`. This revision is local only.

### Combined filters and reader alignment — 2026-09-22
