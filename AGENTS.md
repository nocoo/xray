...
- Local preview starts in Mock mode with seeded data in `packages/worker/.wrangler/state-mock`. The header labels are **MOCK** / **PROD** and can switch to Product through the local development proxy; `bun run login:product` authenticates with Cloudflare Access. Product operations use the signed-in production identity and can change live data. Keep its credentials server-side, and never target Product in automated tests.
