...
### Added
- Dual-host production: browser `xray.hexly.ai` (Access) and ingest `xray-ingest.hexly.ai` (Bearer push)
...
- Playwright L3 dual-host smoke skeleton (`e2e/*.pw.ts`)
...
### Security
- Access JWT on browser host; ingest Bearer-only; exact Origin checks
- Secrets at rest via `XRAY_SECRETS_KEK` (required for AI / zhe.to in production)
- Ingest rate limit binding `XRAY_INGEST_RL`

### Notes
- Auto AI on ingest / Cron X pull / historical `fetched_posts` migration intentionally out of scope
- L3 E2E needs local or staging UI+worker; CI hard gate remains Vitest L2
