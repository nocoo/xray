...
25. **next-auth `new NextRequest(url, req)` drops POST method/body in production** — vinext/Next production builds treat the second constructor arg as plain `RequestInit`, so method becomes GET and body is empty. Auth.js then hits `pages.signin(providerId)` → `UnknownAction` → redirect `/login?error=Configuration`. Browser `signIn()` expects JSON (`X-Auth-Return-Redirect`) but gets HTML → `Unexpected token '<'`. Affects both our wrapHandler and next-auth's internal `reqWithEnvURL`. Fix: `new NextRequest(req)` / explicit `{ method, headers, body, duplex }`, and call `Auth()` directly with a correctly rewritten origin instead of next-auth `handlers`. Also: vinext catch-all `[...nextauth]` POST body streams hang on `arrayBuffer()`; use explicit `/api/auth/*` routes instead of catch-all.
